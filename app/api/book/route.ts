...
export async function POST(request: Request) {
  try {
    ...
    const userHtml = `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 40px; background-color: #fafafa; border: 1px solid #eaeaea;">
        <div style="text-align: center; margin-bottom: 40px;">
          <h1 style="font-size: 24px; letter-spacing: 4px; font-weight: 400; color: #111; margin: 0;">PREMIUM CHARTERS</h1>
        ...
        <p style="color: #333; font-size: 15px; line-height: 1.6;">
          Thank you for reaching out to Premium Charters. We have received your inquiry regarding <strong>"${trip.busName}"</strong>.
        </p>
        ...
        <p style="color: #333; font-size: 15px; line-height: 1.6; margin-top: 40px;">
          Warm regards,<br/>
          <em style="font-family: serif; font-size: 16px;">The Premium Charters Team</em>
        </p>
      </div>
    `;
    ...
    await transporter.sendMail({
      from: `"Premium Charters Concierge" <${process.env.SMTP_EMAIL}>`,
      to: "devhanshtf@gmail.com", // Fixed requested admin email
    ...
    await transporter.sendMail({
      from: `"Premium Charters Concierge" <${process.env.SMTP_EMAIL}>`,
      to: customer.email,
      subject: "Booking Request Received - Premium Charters",
      html: userHtml,
    });
